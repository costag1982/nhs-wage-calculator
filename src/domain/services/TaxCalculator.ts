...
export class TaxCalculator {
  public static calculateMonthlyPaye(taxablePay: number, taxCode: string): number {
    const cleanCode = taxCode.trim().toUpperCase();

    // 1. Check flat rate / special codes
    const flatRateTax = this.calculateFlatRateTax(taxablePay, cleanCode);
    if (flatRateTax !== null) {
      return flatRateTax;
    }
    ...
    return this.calculateGraduatedTax(taxableAmount);
  }

  private static calculateFlatRateTax(taxablePay: number, code: string): number | null {
  ...
  private static getMonthlyAllowance(code: string): number {
  ...
  private static calculateGraduatedTax(taxableAmount: number): number {
