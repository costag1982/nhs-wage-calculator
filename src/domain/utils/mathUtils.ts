 ...
 */
export const roundCurrency = (val: number): number => roundToDecimals(val, 2);

/**
 * Rounds worked or contracted hours to 2 decimal places.
 */
export const roundHours = (val: number): number => roundToDecimals(val, 2);

/**
 * Rounds hourly rates to 4 decimal places, matching NHS ESR pay scale rate precision.
 */
export const roundHourlyRate = (val: number): number => roundToDecimals(val, 4);
